...
class SimpleChain:
    ...
    def run(self, prompt_type, history: List[Message], **kwargs) -> dict:
        conversation = self.reset_conversation("basic", prompt_type, **kwargs)
        user_question = {
            "role": "user",
            "content": prompt_type["basic"]["user"].format(
        ...
        if kwargs.get("context"):
            conversation = self.reset_conversation("with_context", prompt_type, **kwargs)
            user_question["content"] = prompt_type["with_context"]["user"].format(chatbot_name=self.chatbot_name,
                                                                                  **kwargs)
